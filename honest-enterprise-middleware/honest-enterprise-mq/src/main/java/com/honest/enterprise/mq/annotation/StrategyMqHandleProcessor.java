...
/**
 * 扫描实现 AnnMqTopic注解的实现类，通过枚举建立策略映射关系
 * @date:2021-12-23 10:56:00
 * @author:fanjie
 */
@Component
public class StrategyMqHandleProcessor implements ApplicationContextAware {
    /**
    ...
    public void setApplicationContext(ApplicationContext applicationContext) throws BeansException {
        //获取所有策略注解的Bean
        Map<String, Object> orderStrategyMap = applicationContext.getBeansWithAnnotation(AnnMqTopic.class);
        if(orderStrategyMap!=null&&orderStrategyMap.size()>0){
            for(Map.Entry<String, Object> entry : orderStrategyMap.entrySet()){
                ...
                if(StringUtils.isNoneEmpty(key)) {
                    StrategyMqContext.strategyMap.put(key, strategyClass);
                }
            }
        }
    }
    ...
    private String getStrategyKey(Class<?> strategyClass){
        ...
        if(StringUtils.isNoneEmpty(infName)){
            ...
            String tags = strategyClass.getAnnotation(AnnMqTopic.class).tag();
            return infName+"_"+topic+":"+tags;
        }
        return "";
    }
}
